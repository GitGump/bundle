function DataGrid()
{
	this._ops = {
		...
		headpadding:"0 12px",	// 表格头部padding
		...
		hasSelBox:false,			// 是否有checkbox可选列
		spBlank:false,			// 是否支持显示空行：“您请求的数据为空”
		spAutoEdit:false,		// 是否支持自动切换编辑列
		chkEditFunc:null,		//检查单元格是否可编辑的回调函数
		chkSelBoxFunc:null,		//检查此行是否显示勾选框的回调函数
	...
	if (typeof DataGrid.prototype.init != "function")
	{
		...
		/* 以编辑状态填充tr的Select输入框 */
		DataGrid.prototype._fillEditRow_Select = function(td, itemIndex, index, value, listObj){
		...
		/* 以编辑状态填充tr的Switch输入框 */
		DataGrid.prototype._fillEditRow_Switch = function(td, itemIndex, index, value, listObj)
		{
			var gridSelCon = el("div");
			var switchBg = el("i");
			var switchBall = el("i");
			var selId = this._pgList.switchPre + itemIndex + this._ops.id + index;

			gridSelCon.className = "switchCon switchConHs " + listObj.className;
			gridSelCon.id = selId;
			td.appendChild(gridSelCon);

			switchBg.className = "switchBg";
			gridSelCon.appendChild(switchBg);

			switchBall.className = "switchBall";
			gridSelCon.appendChild(switchBall);
		...
		DataGrid.prototype._fillEditRow_PopVigDiy = function(td, itemIndex, index, value, listObj, dataObj){
			var span, input, popObj = listObj.popObj;
			var showSize;

			var valueInfo = this._getPopVigDiyValueList(listObj, dataObj);
			var valueList= valueInfo[0];
			var isAdd = valueInfo[1];
			...
			else
			{
				span = el("span");
				span.className = "dataGridPopVigDes";
				var showValue = popObj.formatValueCb(valueList);
				span.title = showValue;
				showSize = this._calcShowSize(index, listObj);
				showValue = getStrInMax(showValue, showSize);
				span.innerHTML = htmlEscape(showValue);
				span.onclick = function(){
					listObj.click(itemIndex);
				};
				td.appendChild(span);

				td.setAttribute(popObj.indexDes, popObj.unionValueCb(valueList));
			}
		};

		DataGrid.prototype._getPopVigDiyValueList = function(listObj, dataObj)
		{
		    var isAdd = true;
		    var valueList = [];

		    if ($.isArray(listObj.name))
		    {
			for (var i = 0; i < listObj.name.length; i++)
			{
			    if (dataObj[listObj.name[i]] === undefined)
			    {
				isAdd = false;
				break;
			    }
			    valueList.push(dataObj[listObj.name[i]]);
			}
		    }
		    else
		    {
			if (dataObj[listObj.name] === undefined)
			{
			    isAdd = false;
			}
			else
			{
			    valueList.push(dataObj[listObj.name]);
			}
		    }
		...
		DataGrid.prototype._fillEditRow_EditBtns = function(dataObj, td, index){
			...
			save.innerHTML = btn.ok;
			save.className = 'save';
			var cancel = document.createElement('button');
			cancel.innerHTML = btn.cancel;
		...
		DataGrid.prototype._fillEditRow = function(tr, itemIndex, data){
			...
			{
				var hasSelBox = true, chkSelBoxFunc = this._ops.chkSelBoxFunc;
				if (typeof(chkSelBoxFunc) == "function")
				{
					var chkSelBox = chkSelBoxFunc(data);
					hasSelBox = (chkSelBox == true) ? true : false;
				}
				...
				if (hasSelBox == true)
				{
					...
					{
						setStyle(td, {width:this._set.IDWidth});
					}
				}
				else
				{
			...
			for (var index in list)
			{
				...
				/* 不可编辑的列直接显示 */
				var editable = true, chkEditFunc = this._ops.chkEditFunc;
				if (typeof(chkEditFunc) == "function")
				{
					var chkEdit = chkEditFunc(item.name, data);
					editable = (chkEdit == true) ? true : false;
				}
				if (false == item.edit || false == editable)
				{
				...
				case "select":	// select 的组织形式是：options:[{str:"xx", value:"xx"}], func:onchangeHandle
					...
					break;
				case "switch":
					this._fillEditRow_Switch(td, itemIndex, index, value, item);
					break;
				case "checkbox":
		...
		DataGrid.prototype._toolSaveHandle_SaveData = function(tr, dataObj, targetIndex, backUpObj){
			...
			for(var index in list)
			{
				...
				if (typeof(chkEditFunc) == "function")
				{
					var chkEdit = chkEditFunc(item.name, dataObj);
					editable = (chkEdit == true) ? true : false;
				}
				if (false == item.edit || false == editable)
				{
				...
				else if (type == "popVigDiy")
				{
					...
					{
					    if (value[i] != dataObj[nameList[i]])
					    {
							isChanged = true;

					    }
					    backUpObj[nameList[i]] = dataObj[nameList[i]];
					    dataObj[nameList[i]] = value[i];
					}
				}
				else
				{
		...
		DataGrid.prototype._setToolBar = function(){
			...
			var lis, gridType, btn, func, asyn;
		...
		/* 处理type="switch" */
		DataGrid.prototype._switchHandle = function(listObj, dataObj, td){
			var val = dataObj[listObj.name];
			var str = "";
		...
		DataGrid.prototype._btnHandle = function(listObj, dataObj, td, index){
			...
			if (subType != undefined)
			{
				...
				case "bind":
					...
					break;
				case "toggle":
					listObj.deActiveText = listObj.deActiveText ? listObj.deActiveText : '+';
					listObj.activeText = listObj.activeText ? listObj.activeText : '-';
					if (td.isToggled) {
						input.value = listObj.activeText;
						input.className = listObj.classNameUn;
					} else {
						input.value = listObj.deActiveText;
						input.className = listObj.className;
					}
					input.onclick = function() {
						if (td.isToggled) {
						...
						listObj.click(index, td, dataObj, listObj);
						td.isToggled = !td.isToggled;
						// console.dir(listObj);
					};
					break;
				case "radio":
					...
					input.onclick = function()
					{
						...
						td.parentNode.style.backgroundColor = '#e6e6e6';
						listObj.click(index, {obj:this}, dataObj, listObj);
					};
				...
				case "checkbox":
					...
					input.onclick = function() {
						if (this.className === listObj.className) {
				...
				case "switch":
					...
					{
						listObj.onStr = label.WDSDHCPOn;
						listObj.offStr = label.WDSDHCPOff;
					}
		...
		/* 处理type="popVigDiy" */
		DataGrid.prototype._popVigHandleDiy = function(listObj, dataObj, td, cellIndex){
			var index = dataObj[listObj.name], popObj = listObj.popObj;
			var value, showSize;
			var valueInfo = this._getPopVigDiyValueList(listObj, dataObj);
			var valueList= valueInfo[0];
			...
			value = popObj.formatValueCb(valueList);
			showSize = this._calcShowSize(cellIndex, listObj);
		...
		DataGrid.prototype._trEdit = function(tr){
			...
			if (this._ops.hasSelBox == true && (tr.cells[0].innerHTML.length != 0))
			{
		...
		DataGrid.prototype._contentFill = function(tr, list, editable, index, dataObj, dataListLen){
			var td, type, styles, head = this._ops.head;
			var input, obj = this;
			var showSize, str;
			...
			if (this._ops.hasSelBox == true)
			{
				/* 调用用户设置的回调函数，看是否要显示选择列 */
				var hasSelBox = true, chkSelBoxFunc = this._ops.chkSelBoxFunc;
				if (typeof(chkSelBoxFunc) == "function")
				{
					var chkSelBox = chkSelBoxFunc(dataObj);
					hasSelBox = (chkSelBox == true) ? true : false;
				}
			...
			if (this._ops.hasID == true)
			{
				...
				if (this._ops.hasHead == false)
				{
					setStyle(td, {width:this._set.IDWidth.width + "px"});
				}
			}
			...
			for (var item in list)
			{
				...
				if (listObj.styles != null)
				{
					setStyle(td, listObj.styles);
				}
				...
				case "select":
					...
					break;
				case "switch":
					this._switchHandle(listObj, dataObj, td);
					break;
				case "checkbox":
				...
				default:
					...
					if (listObj.convert && typeof listObj.convert === 'function') {
						td.innerHTML = listObj.convert(str);
					} else {
						str = (str == undefined ? "" : str.toString());
						if (true == listObj.igHTMLEscape)
						{
							td.innerHTML = getStrInMax(str, showSize);
						}
						else
						{
							td.innerHTML = htmlEscape(getStrInMax(str, showSize));
						}
		...
		DataGrid.prototype._headCreate = function(){
			...
			tr = table.insertRow(-1);
			tr.className = this._set.headClassName;
			...
			for (var prop in head)
			{
				...
				td = tr.insertCell(-1);
				setStyle(td, {padding: this._ops.headpadding});
		...
		DataGrid.prototype._tableSet = function(){
			...
			if (set.styles != null)
			{
				if (typeof set.styles == "string")
				{
					this._table.className += set.styles;
				}
				else
				{
					setStyle(this._table, set.styles);
				}
			}
		...
		DataGrid.prototype._changeTable = function(start, rowNum){
			...
			if (this._ops.hasSelBox == true)
			{
				if (this._ops.hasHead == true)
				{
					childNodes[0].cells[0].childNodes[0].checked = false;
				}
